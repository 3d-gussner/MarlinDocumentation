...
function genGcode() {
  ...
  var k_script =  '; ### Marlin K-Factor Calibration Pattern ###\n' +
                  '; -------------------------------------------\n' +
                  '; generated by PrusaSlicer 2.2.0\n' + // Need to add this to get prusaprinters.org to parse the information
                  ...
                  '; Settings Printer:\n' +
                  '; Firmware: ' + FIRMWARE + '\n' +
                  '; printer_model = ' + PRINTER + '\n' + // PrusaSlicer variable needed for prusaprinters.org
                  ...
                  //((FIRMWARE == 'MK3' || FIRMWARE == 'MK3S') ? 'M221 S95\n' : '') +
                  ((FIRMWARE == 'MK3S'  ||  FIRMWARE == 'MK3') ? 'M907' : ';M907') + ((PRINT_QUAL == 'SPEED' || PRINT_QUAL == 'DRAFT' ) ? ' E538' : ' E430' ) + ' ; set extruder motor current\n' +
  ...
  if (USE_LINENO) {
    ...
    for (var i = (K_START * 100); i <= (K_END *100); i += (K_STEP * 100)) {
      if (stepping % 2 === 0) {
        k_script += moveTo(numStartX, numStartY + (stepping * LINE_SPACING), basicSettings) +
                    zHop((HEIGHT_LAYER + Z_OFFSET), basicSettings) +
                    doEfeed('+', basicSettings, (USE_FWR ? 'FWR' : 'STD')) +
                    createGlyphs(numStartX, numStartY + (stepping * LINE_SPACING), basicSettings, Math.round10((i /100), -3)) +
                    doEfeed('-', basicSettings, (USE_FWR ? 'FWR' : 'STD')) +
                    zHop((HEIGHT_LAYER + Z_OFFSET + Z_LIFT), basicSettings);
      }
      stepping += 1;
